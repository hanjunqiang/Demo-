...
//                      @{@"title":[EM_ChatResourcesUtils stringWithName:@"buddy.blacklist"],@"icon":kEMChatIconBuddyBlacklist}];
    ...
    NSLog(@"count===%lu",(unsigned long)_tagArray.count);
    
    //如果头部view中的collectionView中没有添加按钮那么就隐藏头部view中的collectionView
    if (!_tagArray.count) {
//        HEIGH_FOR_SEARCH_BAR就是头部view中的搜索view的高度
        _tableHeader.frame=CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, HEIGH_FOR_SEARCH_BAR);
    }
...
#pragma mark -mrhantableView中顶部的搜索和四个按钮加载
- (void)reloadTagBar{
    CGFloat height =0;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (opposite) {
        //如果有代理，那么就执行代理方法，在代理方法中进行操作，或者跳转到自己设计的VC
...
#pragma mark - EMChatManagerBuddyDelegate获取好友列表成功时的回调
